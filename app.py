...
def index():
    ...
    if request.method == 'POST':
        ...
        if txid:
            return redirect(url_for('transaction_by_id', txid=txid))

    # Render the index page with a form to input txid
    return render_template(INDEX_TEMPLATE, txid=None, poc=None, error=error)
